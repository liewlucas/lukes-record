...
- Storyblok's asset CDN sends no CORS headers, so scene textures go through the same-origin proxy `src/pages/img/[...path].ts` (server route, whitelisted to a.storyblok.com, immutable cache headers). `textureUrl()` in `src/lib/storyblok.ts` rewrites CDN URLs for the scene only; the HTML layer links to the CDN directly.
- Scene tiers (`src/scene/capabilities.ts`): no WebGL2 → nothing rendered; low (phones, software GL) → no shadows or post; mid → bloom, grain, vignette; high → N8AO, bloom, AgX, grain, vignette, DoF only on deep-linked focus. Headless Chromium on this Mac reports the Apple GPU and lands on high.
